...
class SispController extends Controller
{
    ...
    public function insertDataGuru(Request $request)
    {
        ...
        $rules = [
            'sisp_email' => 'required|email|unique:sisp,sisp_email',
        ...
        if ($validator->fails()) {
            ...
            $SispModel->sisp_email = $request->sisp_email;
            ...
            if ($save) {
                ...
                $deleteUsers = User::where('username', $request->username)->delete([]);
                ...
                if ($file) {
                    ...
                    DB::table('sisp')->where('sisp_id', $Guru->sisp_id)->delete([]);
    ...
    public function insertDataGuruM(Request $request)
    {
        Validator::extend('alpha_spaces', function($attribute, $value)
        {
            return preg_match('/^[\pL\s\.\,]+$/u', $value);
        ...
        $validator = Validator::make($request->all(), $rules, [], $attributes);
        if ($validator->fails()) {
            $data['response'] = ['status' => 400, 'response' => 'error','type' => "danger", 'message' => implode(",",$validator->getMessageBag()->all())];
        }else{
            $SispModel = new SispModel();
            ...
            $SispModel->sisp_email = $request->sisp_email;
            $SispModel->sisp_telp = $sisp_telp;
            $SispModel->sisp_setkatpes = $Setkatpes_ps->setkatpes_id;
    ...
    public function showByKoo($id)
    {
        $this->data['Pgn'] = $this->getUser();

        $this->data['MethodForm'] = 'insertData';
        $this->data['IdForm'] = 'sispShowKooAddData';
        $this->data['UrlForm'] = 'sispShowKoo';
        ...
        $Sisp = DB::table('sisp')->join('bag', 'sisp.sisp_bag', '=', 'bag.bag_id');
        if ($this->data['Bagk']->bagk_satker=='1') {
            $Sisp = $Sisp->where('bag_prnt', $this->data['Bagk']->bag_id);
        }else {
            $Sisp = $Sisp->where('sisp_bag', $this->data['Bagk']->bag_id);
        }

        $Sisp = $Sisp->select(['sisp_id', 'sisp_nm'])->get();

        $this->data['Sisp'] = GuruController::setData($Sisp, $this->data);

        $this->data['Survei'] = DB::table('survei')->select(['survei_id', 'survei_thn'])->orderBy('survei_ord')->limit(3)->get();

        $this->data['Agent'] = new Agent;

        return view('guru.listSurvei', $this->data);
    }
}